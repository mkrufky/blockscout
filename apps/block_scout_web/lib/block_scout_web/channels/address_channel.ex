defmodule BlockScoutWeb.AddressChannel do
  ...
  def handle_in("get_balance", _, socket) do
    with {:ok, casted_address_hash} <- AddressHash.cast(socket.assigns.address_hash),
      ...
      {:reply, {:ok, %{balance_card: rendered, balance: address.fetched_coin_balance.value}}, socket}
    else
      _ ->
        {:noreply, socket}
    end
  ...
  def handle_out(
    ...
    case render_balance_card(address, exchange_rate, socket.assigns.locale) do
      {:ok, rendered} ->
        push(socket, "balance", %{balance_card: rendered, balance: address.fetched_coin_balance.value})
        {:noreply, socket}
